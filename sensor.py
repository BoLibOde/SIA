...
# Public buffer: device.py will read sensor.sensor_buffer[-1] like before
sensor_buffer = []   # list of tuples (temp, db, co2, voc, timestamp)
_MAX_BUFFER = 500

# Optional CircuitPython libraries (if installed on Raspberry Pi)
board = None
busio = None
adafruit_ccs811 = None
...
try:
    ...
    busio = _busio
    try:
        import adafruit_ccs811 as _ccs
        adafruit_ccs811 = _ccs
    except Exception:
        adafruit_ccs811 = None
    try:
        _bme_mod = importlib.import_module("adafruit_bme280")
        Adafruit_BME280_I2C = getattr(_bme_mod, "Adafruit_BME280_I2C", None)
    except Exception:
        Adafruit_BME280_I2C = None
    try:
    ...
    _LOG.info("CircuitPython / Blinka not available; sensor module will use simulator fallback.")
...
class SensorHub:
    """
    Encapsulates CCS811 + optional SCD4x + optional BME280.
    Exposes read_once() and close().
    """
    def __init__(self, use_scd=False, use_bme=False):
        ...
        # init CCS811 (required for air-quality measurements)
        ...
        if self.use_scd:
            try:
                ...
                try:
                    # start periodic measurement if library requires it
                    self.scd.start_periodic_measurement()
                except Exception:
                    pass
                # brief wait for first reading
                start = time.time()
    ...
    def read_once(self):
        ...
        - ccs_eco2, ccs_tvoc, scd_co2, scd_temp_c, scd_rh_pct, temp_source
        ...
        # read SCD first (preferred temp source)
        try:
        ...
        # fallback BME for temperature if SCD didn't provide it
        ...
        # apply temp compensation to CCS811 if we have a temp
        try:
            if result["scd_temp_c"] is not None:
                try:
...
def _sensor_loop(poll_interval, use_scd, use_bme):
    ...
    # Try to init SensorHub if environment allows
    ...
    while not _stop_event.is_set():
        ...
        if _hub_available and _sensor_hub is not None:
            try:
                ...
                db = random.uniform(35.0, 55.0)  # no dB sensor; keep simulator range
...
def start(poll_interval=2.0, use_scd=False, use_bme=False):
    """
    Start the sensor reader thread. Safe to call multiple times (no-op if already running).
    """
...
def stop():
    """
    Stop the sensor thread and perform cleanup.
    """
    global _sensor_thread, _stop_event
    _stop_event.set()
